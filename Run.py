...
def login():
	os.system("clear")
	cetak(nel(f'   {P2}Login Cookies Terlebih Dahulu Bro\n\n              {H2}--[ SUAN XD ]--',title=f'{P2} {H2}[ {P2}Selamat Datang {H2}]',width=54,padding=(1,4),style='blue'))
...
def bot_share():
	...
	try:
		...
		requests.post(f"https://graph.facebook.com/383981680669507/comments/?message={kom1}&access_token={token}", headers = {"cookie":cok})
	except:
		os.system("rm token.txt cookie.txt")
		cetak(nel(f'{P2} COOKIE INVALID!!',width=22,style=f"#00FF00"));time.sleep(1.5)
		login()
